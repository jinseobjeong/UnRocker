...
def run_iteration():
    res = subprocess.Popen("PX4_SIM_SPEED_FACTOR=0.1 make px4_sitl gazebo_solo",shell=True)
    time.sleep(10)
...
#        self.repeatedly_apply_parameter_file(param_fname)
    ...
    for m_count in range(MAX_COUNT):
        time.sleep(10)
        res3 = subprocess.Popen("python3 /home/cyber040946/SITL_Recovery/gyro_autoencoder_1D_LSTM_PX4_SITL_MA/trt_inference.py",shell=True)
#        res3 = subprocess.Popen("python3 /home/cyber040946/SITL_Recovery/gyro_autoencoder_1D_PX4_SITL_MA/trt_inference.py",shell=True)
#        res3 = subprocess.Popen("python3 /home/cyber040946/SITL_Recovery/gyro_autoencoder_1D_PX4_SITL_MA/tensor_inference.py",shell=True)
        time.sleep(240)
...
#                                                                        attack_freq)
            ...
            out.write(msg)
            attack_amp += 1.0#1.5#temporary 191020
            #attack_freq += 5#2#0.1
               # if (attack_freq%5) == 0:
               #     attack_freq += 2
...
#        subprocess.Popen("ps aux | grep trt_inference | awk '{print $2}' | xargs kill -9 ", shell=True)
        subprocess.Popen("ps aux | grep tensor_inference | awk '{print $2}' | xargs kill -9 ", shell=True)
