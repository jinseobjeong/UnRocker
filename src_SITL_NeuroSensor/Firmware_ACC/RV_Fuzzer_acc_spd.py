...
#    else:
#        print('MAVLINK 20')
    vehicle.parameters['attacc_trigger'] = 1.0
    print "attack : %s" %vehicle.parameters['attacc_trigger']
...
##reboot SITL dmesg | grep "tty" | awk '{print $4}'
def check_tty():
    tty_check = commands.getstatusoutput('dmesg | grep tty | awk \'{print $4}\' ')#")#subprocess.Popen("dmesg | grep tty | awk '{print $4}'", shell=True )
    tty_check_list = list(tty_check)
    A = (tty_check_list[1][-2] == '0')
    print( 'TTY Result : %s' % tty_check[1][-2] )
    print(tty_check)
    print(list(tty_check))
    print(type(tty_check))
    print(tty_check_list[1][-2])
    print( 'results : %s' % A)#tty_check_list[1][-2] == '0')
...
#        res3 = pexpect.spawn("/home/sysseclab/anaconda3/envs/tf3/bin/python ~/prj_jsjeong/accel_autoencoder_1D_PX4_SITL_MA/trt_inference_p2.py")
#        res3 = subprocess.run(['/home/sysseclab/anaconda3/envs/tf3/bin/python', '~/prj_jsjeong/accel_autoencoder_1D_PX4_SITL_MA/trt_inference_p2.py'],shell=True)

        res3 = subprocess.Popen("/home/sysseclab/anaconda3/envs/tf3/bin/python ~/prj_jsjeong/accel_autoencoder_1D_PX4_SITL_MA/trt_inference_p2.py",shell=True)
        time.sleep(5)
        #####
        vehicle.parameters['attacc_trigger'] = 0
...
#        PX4setMode(MAV_MODE_AUTO)
        ...
        time.sleep(30)
        vehicle.parameters['attacc_amp'] = attack_amp*20
        vehicle.parameters['attacc_freq'] = attack_freq
        vehicle.parameters['attacc_trigger'] = 1
...
#                                                                        attack_freq)
            msg = 'crashed SIM_gyro ATTACC at CUTOFF 20, D 0.5, AMP: %f, FREQ: %d\n' % (attack_amp*20, attack_freq)
            print(msg)
            out.write(msg)
            attack_amp -= 0.5#temporary 191020
#            attack_freq += 5#2#0.1
               # if (attack_freq%5) == 0:
               #     attack_freq += 2
...
#                msg = 'Crashed INS_All ATTACK at CUTOFF 5, AMP: 4.500000, FREQ: %d\n' % (attack_freq)
            msg = 'Not Crashed all SIM_gyro ATTACK at CUTOFF 20, D 0.5, AMP: 20, FREQ: %d\n' % (attack_freq)
...
#        self.progress(msg)
        ...
        subprocess.Popen("ps aux | grep trt_inference_p2 | awk '{print $2}' | xargs kill -9 ", shell=True)
