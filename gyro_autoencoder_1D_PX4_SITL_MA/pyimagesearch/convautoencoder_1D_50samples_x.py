...
#		np.take(all_dataset, np.random.permutation(all_dataset.shape[0]),axis=0,out=all_dataset)
		...
		for index in range(testset_h1):
			for sub_index in range(length):
				test_dataset[index][sub_index][0] = at_data1[index+sub_index][6]
				test_dataset[index][sub_index][1] = at_data1[index+sub_index][9]

		np.take(test_dataset, np.random.permutation(test_dataset.shape[0]),axis=0,out=test_dataset)

		for index in range(testset_h1):
			for sub_index in range(length):
				test_attack[index][sub_index] = test_dataset[index][sub_index][0]# at_data1[index+sub_index][6]
				test_gyro[index][sub_index] = test_dataset[index][sub_index][1]#at_data1[index+sub_index][9]
