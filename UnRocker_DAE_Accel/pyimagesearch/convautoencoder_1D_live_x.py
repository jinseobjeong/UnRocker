...
#		print('original csv size %d x %d' %(all_h, all_w))
		...
		at_data1 = np.genfromtxt("DNN_Training/Pixhawk4_real_eval3_attacc_status_0.csv",delimiter=",",dtype=np.float32, skip_header=True)[:,1:]
		at_h1 = at_data1.shape[0]
		at_w1 = at_data1.shape[1]
		testset_h1 = int((at_h1-length)/step_size)
		...
		test_dataset = np.empty((testset_h,length,2))
		index_temp = 0
		offset_attack = 0#0.493
		offset_benign = 0#-1.027

		for index in range(testset_h1):
			for sub_index in range(length):
				test_dataset[index+index_temp][sub_index][0] = at_data1[index*step_size+sub_index][7]-offset_attack
				test_dataset[index+index_temp][sub_index][1] = at_data1[index*step_size+sub_index][10]-offset_benign
...
#		print('original csv size %d x %d' %(all_h, all_w))
		...
		for index in range(testset_h1):
			for sub_index in range(length):
				test_attack[index][sub_index] = at_data1[index+sub_index][7] - offset_attack
				test_gyro[index][sub_index] = at_data1[index+sub_index][10] - offset_benign
